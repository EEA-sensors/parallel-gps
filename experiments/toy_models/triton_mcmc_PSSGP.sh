...
echo "Running PSSGP_${mcmc}_${cov}"

python -m $py_script --step_size=$step_size --n_runs=$n_runs --mcmc=$mcmc --model=PSSGP --cov=$cov --rbf_order=6 --rbf_balance_iter=10 --qp_order=6 --data_model=SINE --noise_variance=0.1 --dtype=float32 --device=$device
