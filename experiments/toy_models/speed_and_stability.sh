...
for cov in Matern32 Matern52 RBF; do
  ...
  python -m $py_script --model=VGP --cov=$cov --rbf_order=6 --rbf_balance_iter=10 --qp_order=6 --data_model=SINE --noise_variance=0.1 --dtype=float64 --device="/gpu:1" --mesh_size=$mesh_size
  sleep 10s
done
