...
for mcmc in HMC MALA NUTS; do
  for cov in Matern32 Matern52 RBF; do
    ...
    sleep 10s
    python -m $py_script --step_size=$step_size --n_runs=$n_runs --mcmc=$mcmc --model=GP --cov=$cov --rbf_order=6 --rbf_balance_iter=10 --qp_order=6 --data_model=SINE --noise_variance=0.1 --dtype=float32 --device="/gpu:0"
    sleep 10s
  done
done
